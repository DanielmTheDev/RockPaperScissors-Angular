...
export class PlayerCreationService {
  ...
  createPlayer(roomId: string): Observable<string> {
    const dialogRef = this.dialog.open(PlayerCreationModalComponent, {
      width: constants.dialogWidth,
      disableClose: true
    });
    return dialogRef.afterClosed().pipe(switchMap(name => this.persistPlayer(name, roomId)));
  }

  private persistPlayer(name: string, roomId: string): Observable<string> {
    return this.initializeBasicPlayer(name, roomId)
